...
# Fix import
content = content.replace("import kotlin.random.Random\n\n// --- Added for PYQ Generator ---", "// --- Added for PYQ Generator ---")
content = "import kotlin.random.Random\n" + content

# Fix the string literals causing issues. I will replace the tricky strings with simple ones for now.
# Replace the blocks using regex.

import re

# We will just rewrite the `generatePyqQuestions` function entirely correctly.
pattern = re.compile(r"// --- Added for PYQ Generator ---[\s\S]*?return list\n}", re.MULTILINE)

fixed_generator = """// --- Added for PYQ Generator ---
...
content = pattern.sub(fixed_generator, content)
